...
def acs(r):
    ...
    # If relayState param is passed, use that else consider the previous 'next_url'
    ...
    if settings.SAML2_AUTH.get("USE_JWT") is True:
        ...
        query = f"?uid={target_user.id}&access={access_token}&refresh={refresh_token}"

        # if available pass the next parameter back to the frontend
        if next_url:
